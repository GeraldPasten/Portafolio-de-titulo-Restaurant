...
function order_list_functionality(){
  onRowClick("orders_table", function (row){
    ...
    if ( user_is_super && row.classList.contains("mark-as-complete") ){
      var r = confirm("El pedido "+id+" se encuentra listo?");
      if (r == true) {
        $.ajax({
...
function check_user_super(){
  ...
  $.ajax({
       ...
       success: function(res) {
            ...
            if (res == "True"){
              console.log("Asignado como verdadero")
              return_value = true;
            }else{
              return_value = false;
            }
...
function load_cart(){
  ...
  document.getElementById('cart_heading').innerHTML = "Para remover un pedido, has click en él"
  ...
  if (cart !== null && cart.length > 0){
    ...
    onRowClick("cart_body", function (row){
      ...
      var r = confirm("Eliminar '"+description+ "' del carrito?");
      if (r == true) {
        document.getElementById("cart_body").deleteRow(value-1);
        $.ajax({
         url: "/restaurant/restar/"+value+"/",
         type: 'GET',
         data: {'value':value}
        });
        //edit the cart
...
function display_empty_cart(){
  ...
  document.getElementById('cart_heading').innerHTML = "Carrito esta Vacio!"
...
function retrieve_saved_cart(){
  if (localStorage.getItem("cart_retrieved") !== "true"){
    $.ajax({
         url: "/restaurant/retrieve_saved_cart",
         type: 'GET',
         success: function(res) {
